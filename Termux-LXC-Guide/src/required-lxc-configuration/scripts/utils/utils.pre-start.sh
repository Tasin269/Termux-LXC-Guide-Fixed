...
sed -i -E 's/^( *#* *)?DNS=.*/DNS=8.8.8.8/g' "${LXC_ROOTFS_PATH}/etc/systemd/resolved.conf"
...
if ! grep -Eq "^# RESET_PASSWORD_ONCE=done" "${LXC_CONFIG_FILE}"; then
  ...
  LD_PRELOAD= chroot "${LXC_ROOTFS_PATH}" usr/bin/sh -c " \
    ...
    useradd -m tst -s /bin/bash; \
    echo 0000 | sed 's/.*/\0\n\0/' | passwd tst 2>/dev/null >/dev/null; \
    echo 'tst ALL=(ALL) NOPASSWD: ALL' >> /etc/sudoers; \
    '/tmp/${CONFIG_BASENAME}/src/required-lxc-configuration/scripts/utils/utils.temp-mount.sh' umount; \
  "
...
bash "${GITHUB_DIR}/src/required-lxc-configuration/scripts/utils/utils.storage-mount.sh" mount
